...
class IsolationForestDetector:
    ...
    def fit(self, df_baseline: pd.DataFrame):
        ...
        print(
            f"  [IForest Calibration] Baseline score mean: {mean_score:.4f}, std: {std_score:.4f}. Threshold set to: {self.score_threshold:.4f}"
        )
...
class RRCFDetector:
    ...
    def fit(self, df_baseline: pd.DataFrame):
        ...
        print(
            f"  [RRCF Calibration] Baseline score mean: {mean_score:.4f}, std: {std_score:.4f}. Threshold set to: {self.score_threshold:.4f}"
        )
    ...
    def _compute_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        scale = np.nan_to_num(stds, nan=0.0) + 1e-5 * (
            np.nan_to_num(means, nan=0.0) + 1.0
        )
        ...
        while len(forest) < num_trees:
            if n // tree_size > 0:
                ...
                ixs = np.random.choice(
                    n, size=(n // tree_size, tree_size), replace=False
                )
...
class BOCPDDetector:
    ...
    def detect(self, df: pd.DataFrame):
        ...
        df_clean = (
            df.fillna(0).replace([np.inf, -np.inf], np.nan).ffill().bfill().fillna(0)
        )
        ...
        try:
            ...
            R, maxes = online_changepoint_detection(
                data, partial(constant_hazard, 50), MultivariateT(dims=data.shape[1])
            )
            cps = find_cps(maxes)
            anomaly_indices = [p[0] for p in cps]
        except Exception as e:
            print(
                f"  [BOCPD Warning] Failed to run optimized custom BOCPD: {e}. Falling back to empty anomalies."
            )
            anomaly_indices = []
...
def run_metric_anomaly_detection(
    ...
    multivariate_cols = [
        ...
        if "latency" not in c.lower() and "error" not in c.lower()
    ]
    ...
    if USE_BOCPD:
        ...
        mif = IsolationForestDetector(
            threshold_multiplier=IFOREST_MULTIVARIATE_THRESHOLD_MULTIPLIER
        )
    ...
    for col in df_features.columns:
        ...
        if "latency" in col or "error" in col:
            ...
            ewma_results[col] = {"anomalies": anoms, "scores": scores}

    return {
        "multivariate": {"anomalies": mif_anomalies, "scores": mif_scores},
